...
int firstOccurence(int arr[], int n, int key){
    ...
    while (low<=high){
        ...
        if (key==arr[mid]){
            res = mid;         // make it as a candidate of first occurence
            high = mid - 1;       // first occurence will always be on the left side of mid
        }     // make sure no other key is on left side , if so make it as a conadidate and return
        else if (key<arr[mid])
            high = mid-1;
        else
            low = mid +1;
...
int lastOccurence(int arr[], int n, int key){
    ...
    while(low<=high){
        ...
        {
            res = mid;
            low = mid + 1;  // last occurence will always be on the right side of mid
        }                   // check for another possible candidate of last occurence
        else if (key<arr[mid])
            high = mid-1;
        else
            low = mid+1;
